...
def cmd_status(args):
    ...
    if not Path(ws).is_dir():
        ...
        sys.exit(1)
    res = run_status(ws, grid_path=args.grid, out_dir=args.out, project=args.project or "")
    _print_status(res)
...
def cmd_chat(args):
    try:
        ...
        print("可先用：python research_ops.py status --workspace <目录>")
...
def main():
    ...
    ps.add_argument("--workspace", "-w", default="workspace", help="实验项目目录")
    ...
    pc.add_argument("--workspace", "-w", default="workspace")
